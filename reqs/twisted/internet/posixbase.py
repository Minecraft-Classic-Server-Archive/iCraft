...
from reqs.zope.interface import implements, classImplements

from reqs.twisted.python.compat import set
from reqs.twisted.internet.interfaces import IReactorUNIX, IReactorUNIXDatagram
from reqs.twisted.internet.interfaces import IReactorTCP, IReactorUDP, IReactorSSL, _IReactorArbitrary
from reqs.twisted.internet.interfaces import IReactorProcess, IReactorMulticast
from reqs.twisted.internet.interfaces import IHalfCloseableDescriptor
from reqs.twisted.internet import error
from reqs.twisted.internet import tcp, udp

from reqs.twisted.python import log, failure, util
from reqs.twisted.persisted import styles
from reqs.twisted.python.runtime import platformType, platform

from reqs.twisted.internet.base import ReactorBase, _SignalReactorMixin

try:
    from reqs.twisted.internet import ssl
    sslEnabled = True
except ImportError:
    sslEnabled = False

try:
    from reqs.twisted.internet import unix
    unixEnabled = True
except ImportError:
    unixEnabled = False
...
if platformType == 'posix':
    from reqs.twisted.internet import fdesc, process, _signals
    processEnabled = True
...
class PosixReactorBase(_SignalReactorMixin, ReactorBase):
    ...
    def spawnProcess(self, processProtocol, executable, args=(),
        ...
        if platformType == 'posix':
            ...
            if win32process:
                from reqs.twisted.internet._dumbwin32proc import Process
