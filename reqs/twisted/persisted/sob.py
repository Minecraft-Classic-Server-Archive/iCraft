...
try:
    ...
    import StringIO
from reqs.twisted.python import log, runtime
...
class Persistent:
    ...
    def _getStyle(self):
        if self.style == "source":
            from reqs.twisted.persisted.aot import jellyToSource as dumpFunc
            ext = "tas"
        else:
...
def load(filename, style, passphrase=None):
    ...
    mode = 'r'
    if style=='source':
        from reqs.twisted.persisted.aot import unjellyFromSource as _load
    else:
