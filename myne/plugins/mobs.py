...
initfile = open("myne/entities/__init__.py")
exec initfile
initfile.close()
initfile = None
...
def loadentities():
    ...
    datafilelist = os.listdir("myne/entities/")
    ...
    for entry in listofentityfiles:
        entitycodedict[entry[:-3]] = open("myne/entities/%s" % entry)
    ...
    for entry in validentities:
        ...
        if possibeAliasFile in datafilelist:
            for alias in open("myne/entities/%s" % possibeAliasFile):
                alias = alias.rstrip()
                if alias != '':
    ...
    for entry in validentities:
        ...
        if possibeSelectFile in datafilelist:
            entityselectdict[entry] = open("myne/entities/%s" % possibeSelectFile)
            
        possibeCreateFile = entry + "_create.py"
        if possibeCreateFile in datafilelist:
            entitycreatedict[entry] = open("myne/entities/%s" % possibeCreateFile)
