...
class BlockTracker(ProtocolPlugin):
    ...
    #def modechange(self, parts, byuser, overriderank):
    ...
    def commandBlock(self, parts, byuser, overriderank):
    ...
    def commandForce(self, parts, byuser, overriderank):
        self.client.world.BlockEngine.dbwrite()
        
    def commandUndoZone(self, parts, byuser, overriderank):
        if len(parts) == 2:
            if parts[1] == "on":
                self.marker = []
    ...
    def commandUnGrief(self, parts, byuser, overriderank):
        if len(parts) == 2:
            if parts[1] == "on":
                self.marker = []
